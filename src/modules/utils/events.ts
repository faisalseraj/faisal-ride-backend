...
export const ALL_EVENTS = {
  ...
  UserManagementEvents: {
    ...
    promoteDemote: (user: IUserDoc) => {
      ...
      // Determine promotion/demotion message
      switch (user.userType) {
        case 'apartment-complex-employee':
          actionLog += `User demoted to Apartment complex Employee from Apartment complex Manager`;
          if (user?.apartmentComplex) {
            actionLog += ` (apartmentComplexId: ${user.apartmentComplex})`;
          }
          break;
        case 'apartment-complex-manager':
          actionLog += `User promoted to Apartment complex Manager from Apartment complex Employee`;
          if (user.apartmentComplex) {
            actionLog += ` (apartmentComplexId: ${user.apartmentComplex})`;
          }
          break;
        case 'tow-company-employee':
          actionLog += `User demoted to Tow company Employee from Tow company Manager`;
          if (user.towCompanyId) {
            actionLog += ` (towCompanyId: ${user.towCompanyId})`;
          }
          break;
        case 'tow-company-manager':
          actionLog += `User promoted to Tow company Manager from Tow company Employee`;
          if (user.towCompanyId) {
            actionLog += ` (towCompanyId: ${user.towCompanyId})`;
          }
          break;
