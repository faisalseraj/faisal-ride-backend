...
import { LogStatus } from '../logs/log.interfaces';
...
const handleCheckoutSessionCompleted = async (session: Stripe.Checkout.Session) => {
  try {
    ...
    await logService.logPayment(null, 'Checkout session completed', {
...
const handleSubscriptionCreated = async (subscription: Stripe.Subscription) => {
  try {
    ...
    await logService.logPayment(user, 'Subscription created via webhook', {
      status: LogStatus.SUCCESS,
      metadata: {
        stripeSubscriptionId: subscription.id,
        stripeCustomerId: customerId,
...
const handleSubscriptionUpdated = async (subscription: Stripe.Subscription) => {
  try {
    ...
    await logService.logPayment(null, 'Subscription updated via webhook', {
      status: LogStatus.SUCCESS,
      metadata: {
        stripeSubscriptionId: subscription.id,
        status: subscription.status,
...
const handleSubscriptionDeleted = async (subscription: Stripe.Subscription) => {
  ...
  try {
    ...
    await logService.logPayment(null, 'Subscription canceled via webhook', {
...
const handleInvoicePaymentSucceeded = async (invoice: Stripe.Invoice) => {
  try {
    ...
    await logService.logPayment(user, 'Subscription payment succeeded', {
      status: LogStatus.SUCCESS,
      metadata: {
...
const handleInvoicePaymentFailed = async (invoice: Stripe.Invoice) => {
  try {
    ...
    // Log failed payment
    await logService.logPayment(user, 'Subscription payment failed', {
      status: LogStatus.FAILED,
      metadata: {
        stripeSubscriptionId: subscriptionId,
        invoiceId: invoice.id,
...
const handleInvoiceUpcoming = async (invoice: Stripe.Invoice) => {
  try {
    ...
    if (daysUntilDue <= 7 && tier) {
      ...
      // Log reminder sent
      await logService.logPayment(user, `Subscription reminder sent - ${daysUntilDue} days until renewal`, {
        status: LogStatus.SUCCESS,
        metadata: {
          stripeSubscriptionId: subscriptionId,
          invoiceId: invoice.id,
...
const handleInvoiceCreated = async (invoice: Stripe.Invoice) => {
  try {
    ...
    await logService.logPayment(user, 'Invoice created for subscription', {
      status: LogStatus.SUCCESS,
      metadata: {
        invoiceId: invoice.id,
        subscriptionId: (invoice as any).subscription,
