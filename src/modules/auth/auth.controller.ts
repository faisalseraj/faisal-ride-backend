...
export const setupEmailAndPassword = catchAsync(async (req: Request, res: Response) => {
  ...
  await userService.updateUserById(
    ...
    'english',
    {
      skipEmailVerificationLinkGeneration: true,
    }
  );
...
export const verifyPhoneNumber = catchAsync(async (req: Request, res: Response) => {
  ...
  // Update the requesterPhoneNumber in all tow requests created by this user
  // This ensures the tow request has the verified phone number
  if (loggedInUser.userType === 'tow-requester') {
    await TowRequest.updateMany(
      ...
      { $set: { requesterPhoneNumber: targetPhoneNumber } }
    );
    console.log(`Updated requesterPhoneNumber in tow requests for user ${loggedInUser.id} to ${targetPhoneNumber}`);
  }
